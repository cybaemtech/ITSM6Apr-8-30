        ...
        {activeTab === 'users' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center px-2">
              <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Staff Management ({users.length})</h4>
              {selectedEngineers.length > 0 && (
                <button 
                  onClick={handleNotifyClients}
                  disabled={notifying}
                  className="bg-red-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-red-500/30 flex items-center gap-2 active:scale-95 transition-all"
                >
            ...
            {users.map(u => {
              ...
              return (
                <div 
                  ...
                  className={`p-5 rounded-[2rem] border shadow-sm flex items-center justify-between transition-all ${isSelected ? 'bg-red-50 border-red-200' : 'bg-white border-slate-200'} ${isEngineer ? 'cursor-pointer active:scale-98' : ''}`}
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black ${isSelected ? 'bg-red-600 text-white' : 'bg-slate-50'}`}>
                      {u.name.charAt(0)}
                    </div>
                    <div>
                      <p className="text-sm font-extrabold text-slate-900 uppercase leading-none">{u.name}</p>
                      <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">{u.role} • {u.email}</p>
                    </div>
                  </div>
                  {isEngineer && (
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${isSelected ? 'bg-red-600 border-red-600' : 'border-slate-200'}`}>
                      {isSelected && <div className="w-2 h-2 bg-white rounded-full" />}
                    </div>
                  )}
                  {!isEngineer && <ChevronRight className="w-4 h-4 text-slate-300" />}
                </div>
              );
            })}
          </div>
